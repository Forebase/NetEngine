...
async def _diagnose(spec_file: str, as_json: bool) -> None:
    ...
    for r in results:
        ...
        phase = f"Phase {r.phase} " if r.phase is not None else ""
        resource = f": {r.resource}" if r.resource else ""
        ...
        if r.logs and actionable:
            for log_command in r.logs:
                click.echo(f"{'':14}  Try: {log_command}")
        if r.retry_command and actionable:
            click.echo(f"{'':14}  Try: {r.retry_command}")
